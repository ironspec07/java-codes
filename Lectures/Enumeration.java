 ...
 * - Enum is named constants that we create for specific constants or error
 * codes.
 ...
 * - In enum, instead of returning a string format or a specific object, we can
 * simply return a status or a value present inside an enum.
 * e.g., enum Status{
 * Running, Failed, Pending, Success;
 * }
 * - enum is also treated as a class in Java. Objects can also be created for
 * enum like:
 * Status s= Status.Running;
 ...
 * - ordinal() method is used to get the index of the name constant present in
 * the enum.
